...
from ORM.schemas import SMailing, SStudyMarkIn
...
class MarkMailer:
    ...
    async def send_study_mail(self,
                              telegram_id: int | str,
                              template_name: str,
                              mark_schemas: Iterable[SStudyMarkIn]):
    ...
    async def send_study_by_attrs(self,
                                  modes: list[SMode],  # режим (поле для фильтра, фильтр)
                                  mark_schemas: Iterable[SStudyMarkIn],  # список оценок (измененных, добавленных, удаленных)
        ...
        for mode in modes:
            ...
            for sorted_mail_value in sorted_mail_values:
                sorted_mark_schemas: list[SStudyMarkIn] = [*filter(
                ...
                if sorted_mark_schemas and sorted_mailing_schemas:
                    ...
                    for sorted_mailing_schema in sorted_mailing_schemas:
                        attr_tasks.append(self.send_study_mail(
    ...
    async def mail_study(self, analyze: SAnalyze):
        ...
        await self.send_study_by_attrs(
